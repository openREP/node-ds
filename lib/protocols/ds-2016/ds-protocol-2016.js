...
class DSProtocol2016 extends DSProtocolBase {
    ...
    readRobotToClientPacket(data) {
        ...
        const seq = data.readUInt16BE(0);
        const voltage = ProtoUtils.decodeVoltage(data.readUInt8(4), 
                                                 data.readUInt8(5));
    ...
    _setupClientSockets() {
        ...
        this.d_clientToRobotSocket.on('data', (msg, rinfo) => {
            const pktInfo = this.readRobotToClientPacket(msg);
            if (pktInfo) {
    ...
    _setupClientTimers() {
        this.d_clientToRobotSendTimer = setInterval(() => {
            ...
            const pktBuf = this.makeClientToRobotPacket(robotPacket);
            if (pktBuf) {
