 ...
 * @property {boolean} hasCode - Whether robot code is running
...
function makeRobotToClientPacket(packetInfo) {
    ...
    packet.writeUInt8(packetInfo.hasCode ? ProtoConstants.ROBOT_HAS_CODE : 0, 4);
    packet.writeUInt8(upper, 5);
    packet.writeUInt8(lower, 6);
    packet.writeUInt8((packet.seq === 0 ? 1 : 0), 7);
...
function readRobotToClientPacket(data) {
    ...
    const seq = data.readUInt16BE(0);
    const hasCode = !!(data.readUInt8(4) & 0x20);
    const voltage = decodeVoltage(data.readUInt8(5),
                                  data.readUInt8(6));
    const requestDate = !!data.readUInt8(7);
