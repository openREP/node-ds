...
class DSProtocolBase extends EventEmitter {
    constructor() {
        ...
        this.d_shouldRebootController = false;
        this.d_shouldRestartCode = false;
    ...
    /**
     * Indicate if the robot controller should be rebooted
     * (CLIENT ONLY)
     * @property {boolean} shouldRebootController
     */
    get shouldRebootController() {
        return this.d_shouldRebootController;
    }
    set shouldRebootController(val) {
        if (val !== this.d_shouldRebootController) {
            this.d_shouldRebootController = val;
            this.emit('stateChanged', {
                field: 'shouldRebootController',
                value: val
            });
        }
    }
    ...
    /**
     * Indicate if the robot code should be restarted
     * (CLIENT ONLY)
     * @property {boolean} shouldRestartCode
     */
    get shouldRestartCode() {
        return this.d_shouldRestartCode;
    }
    set shouldRestartCode(val) {
        if (val !== this.d_shouldRestartCode) {
            this.d_shouldRestartCode = val;
            this.emit('stateChanged', {
                field: 'shouldRestartCode',
                value: val
            });
        }
    }
    ...
    */
    ...
    // === Protocol Lifecycle ===
    // The protocol itself should handle its state and update
    // the outside world accordingly
    startAsClient() {
