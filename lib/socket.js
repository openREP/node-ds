...
class DSSocket extends EventEmitter {
    constructor(config) {
        ...
        if (config.inPort !== undefined) {
            ...
            this.d_inSock.on('error', (err) => {
                this.emit('inSockError', err);
            });

            this.d_inSock.on('message', (msg, rinfo) => {
                // Forward this up
                this.emit('inSockData', msg, rinfo);
            });
        ...
        if (config.outPort !== undefined) {
            ...
            this.d_outSock.on('error', (err) => {
                this.emit('outSockError', err);
            });

            this.d_outSock.on('message', (msg, rinfo) => {
                this.emit('outSockData', msg, rinfo);
            });
        }
    ...
    send(buf) {
        ...
        var bytesWritten = 0;
        var len = buf.length;
        this.d_outSock.send(buf, this.d_outPort, this.d_remoteAddress, (err, numBytes) => {
